    ...
    <div className="modal-overlay" onClick={onClose} style={{ zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '400px', width: '90%', padding: '1.5rem', borderRadius: '12px', background: 'white' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Typ služby:</label>
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Čas OD:</label>
              <input
                type="time"
                value={timeFrom}
                onChange={e => setTimeFrom(e.target.value)}
                required
                style={{ width: '100%', padding: '0.7rem', borderRadius: '8px', border: '2px solid var(--info-border-soft)', fontSize: '1rem', boxSizing: 'border-box' }}
              />
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Čas DO:</label>
              <input
                type="time"
                value={timeTo}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem', marginTop: '0.5rem' }}>
             <div>
               <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 600, textAlign: 'center' }}>Velitel (1-2)</label>
               <input type="number" min="1" max="2" value={velitelCount} onChange={e => setVelitelCount(parseInt(e.target.value)||1)} style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--border)', textAlign: 'center', boxSizing: 'border-box' }} />
             </div>
             <div>
               <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 600, textAlign: 'center' }}>Strojník (1-2)</label>
               <input type="number" min="1" max="2" value={strojnikCount} onChange={e => setStrojnikCount(parseInt(e.target.value)||1)} style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--border)', textAlign: 'center', boxSizing: 'border-box' }} />
             </div>
             <div>
               <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 600, textAlign: 'center' }}>Hasič (2-5)</label>
               <input type="number" min="2" max="5" value={hasicCount} onChange={e => setHasicCount(parseInt(e.target.value)||2)} style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--border)', textAlign: 'center', boxSizing: 'border-box' }} />
             </div>
          </div>

          {droppedAssignees.length > 0 && (
            <div style={{ background: 'var(--warning-bg)', border: '1px solid var(--accent-amber-grad-1)', borderRadius: '8px', padding: '0.65rem 0.85rem', fontSize: '0.8rem', color: 'var(--warning-dark)' }}>
