    ...
    return (
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '1.1rem 1.25rem', borderBottom: '1px solid var(--border)', background: 'var(--surface-sunken)' }}>
                <h3 style={{ margin: 0, fontSize: '1.05rem', color: 'var(--text-charcoal)' }}>{title}</h3>
            </div>
    ...
    return (
        <div style={{
            ...
            {label !== undefined && label !== '' && (
                <div style={{ fontWeight: 600, marginBottom: '0.3rem', color: 'var(--text-charcoal)' }}>{label}.</div>
            )}
            {payload.map((entry, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--text-charcoal)', marginTop: i > 0 ? '0.2rem' : 0 }}>
