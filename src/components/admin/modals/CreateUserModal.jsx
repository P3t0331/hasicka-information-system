        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '480px' }}>
                ...
                <div className="modal-body">
                    <div style={{
                        padding: '0.75rem', background: '#FFF8E1', borderRadius: '8px',
                        border: '1px solid #FFB300', fontSize: '0.85rem', color: '#795548',
                    ...
                    <form onSubmit={handleSubmit}>
                        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
                            <div style={{ flex: 1 }}>
                                <label style={{ display: 'block', marginBottom: '0.3rem', fontWeight: 600, fontSize: '0.85rem', color: '#555' }}>
                                    Jméno *
                                </label>
                                <input
                                    type="text" value={firstName} onChange={e => setFirstName(e.target.value)} required
                                    autoFocus
                                    style={{ width: '100%', padding: '0.65rem 0.75rem', borderRadius: '8px', border: '1px solid #e0e0e0', fontSize: '0.9rem', boxSizing: 'border-box' }}
                                />
                            </div>
                            <div style={{ flex: 1 }}>
                                <label style={{ display: 'block', marginBottom: '0.3rem', fontWeight: 600, fontSize: '0.85rem', color: '#555' }}>
                        ...
                        <div style={{ marginBottom: '1.1rem' }}>
                            ...
                            <div style={{ position: 'relative' }}>
                                <input
                                    ...
                                    required
                                    style={{ width: '100%', padding: '0.65rem 2.5rem 0.65rem 0.75rem', borderRadius: '8px', border: '1px solid #e0e0e0', fontSize: '0.9rem', boxSizing: 'border-box' }}
                                />
                                <button type="button" onClick={() => setShowPassword(v => !v)} style={{
                                    position: 'absolute', right: '0.6rem', top: '50%', transform: 'translateY(-50%)',
                                    background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.9rem', color: '#888', padding: 0
                                }}>
                                    {showPassword ? '🙈' : '👁️'}
                                </button>
                            </div>
                        ...
                        <div style={{ marginBottom: '1.25rem' }}>
                            <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, fontSize: '0.85rem', color: '#555' }}>
                                Role
                            </label>
                                ...
                                {ROLE_OPTIONS.filter(r => r !== 'Admin').map(role => (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            background: roles.includes(role) ? '#1565C0' : '#f5f5f5',
                                            color: roles.includes(role) ? 'white' : '#555',
                                            border: roles.includes(role) ? '1px solid #1565C0' : '1px solid #ddd'
                                        }}
                                    >
                                        {role}
                                    </button>
                        ...
                        {error && (
                            <div style={{
                                padding: '0.75rem', background: '#FFEBEE', borderRadius: '8px',
